...
The table-headers sidecar (roadmap item 1), through step 3 of its build: the sidecar, the key, and the report exist; the conversion applies the value in effect for every data table; and a merged title row becomes the caption. Output changes for tables declared or guessed to have a header column, an unmarked header row, no headers, or a title row.
