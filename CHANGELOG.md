...
- `util/restyle-headings.py`, a source repair: reports the paragraph styles a DOCX uses and rewrites its heading styles from a map, from `--promote STYLE`, or from the levels the document's own TOC field declares. Pandoc's DOCX reader turns every `Title` paragraph after the first into a plain paragraph, so a book styled `Title` for its modules arrives with no modules; this is what fixes that before conversion. Empty paragraphs of a remapped style are dropped.
