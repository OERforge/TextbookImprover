...
**What a DOCX round trip loses, measured on four books.** A list inside a block quote comes back outside it; a table's role is gone, since Word keeps no layout-table mark; a figure with no caption comes back as an image. With the two repairs above, DCIC (from HTML) survives unchanged on 44 of 80 pages, and the statistics book (from Word) on 135 of 169.
