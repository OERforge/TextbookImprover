...
**What a DOCX round trip loses, measured on four books.** Nested block quotes flatten to one level, since the writer gives every level the `Block Text` style; a list item holding a code block or a second paragraph splits the list, the code or the paragraph coming back as an item of its own (measured with Pandoc alone, too), and a list inside a block quote comes back with each item's text in a quote of its own; a table's role is gone, since Word keeps no layout-table mark. On DCIC, from HTML, 32 of 80 pages survive unchanged; on the statistics book, from Word, 131 of 169.
