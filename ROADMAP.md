...
**Status: step 3 of 4 landed.** The sidecar, the key, the pre-pass, and the report exist; the filter applies the value in effect (`first-row`, `first-column`, `both`, `none`; a blank, `manual`, and `list` leave the table as Pandoc gave it); and `caption-rows` folds named rows into the caption, with `caption-rows=1` inferred for a merged title row. Next: `split-at`.
