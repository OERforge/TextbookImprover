...
- **An indent for each level of a quotation**, and for the code inside one, which Pandoc's writer gives every level alike, with a blank paragraph between two quotations in a row, which Pandoc's reader would otherwise join.
...
The post-processing finds what it changes by marks the target puts on the elements before Pandoc writes them, bookmarks around each table, decorative image, and quotation, never by counting: a figure holding two images is written as a table, so a page's third Word table can be its second table. The marks are removed afterward.
