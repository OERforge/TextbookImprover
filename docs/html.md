...
An EPUB is unpacked into pages by [`unpack-epub.py`](epub-input.md). Pages saved from a website arrive with their navigation, their scripts' leftovers, links that point at the live site, and no order in their file names; reading the order from the pages' own menus, stripping what isn't the book, and unpacking an `.mhtml` set are the rest of the roadmap's first item. `table-headers.csv` and the census's guess don't reach an HTML table yet, so one with no `<th>` is reported and stays as it is.
