...
- A formula as MathJax 2 drew it, with no TeX beside it (what a browser's `.mhtml` save keeps), is rebuilt as MathML: each span MathJax drew for a MathML element names it in its class and is numbered in the formula's order, so the structure is all there. Where MathJax's `<script type="math/tex">` follows the rendering, the rendering goes instead and the TeX is read. Without this, Pandoc read a formula as spans nested ten deep, which a Markdown target wrote faithfully and couldn't read back.
