...
- The title in a Pandoc-written page's title block is given an id before reading. Pandoc's reader makes one from a heading's text when it has none, and a page's title is often its first section's heading too, so the two ids collided and Pandoc warned on nearly every page of DCIC read back.
- A row of column headers partway down a table (`<th scope="col">` cells, after rows of data) makes a second table from there, headed by that row: the source is two tables, one under the other. Pandoc has no header cells in a table's body, so the row would otherwise be read as data. A `scope` Pandoc can't place is dropped; the filter writes its own.
- An attribute whose name can't be one, left by a broken editor (EdTech Books' leaves CSS as a name), is dropped.
